...
namespace :events do
  ...
  task :export_to_s3, [:created_before, :created_on_or_after] => :environment do |_, args|
    created_before = args[:created_before] ? DateTime.parse(args[:created_before]) : 2.months.ago.beginning_of_week(:sunday)
    created_on_or_after = args[:created_on_or_after] ? DateTime.parse(args[:created_on_or_after]) : nil
